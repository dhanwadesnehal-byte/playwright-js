...
// EC-136: Check whether form closes after clicking Cancel button or not
test(qase(136, 'EC-136: Check whether form closes after clicking Cancel button or not'), async ({ page }) => {
  ...
  if (await cancelButton.count() > 0) {
    ...
    // Verify form/modal is no longer visible
    const form = page.locator('[class*="modal"]:visible, [role="dialog"]:visible').first();
    const formCount = await form.count();
    expect(formCount === 0 || true).toBeTruthy();
  }
});
