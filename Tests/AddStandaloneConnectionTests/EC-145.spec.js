...
// EC-145: Check whether form retains data after validation error or not
test(qase(145, 'EC-145: Check whether form retains data after validation error or not'), async ({ page }) => {
  ...
  if (await nameField.count() > 0) {
    const testName = 'Test Connection Retain';
    ...
    const retainedValue = await nameField.inputValue();
    expect(retainedValue).toBe(testName);
  }
});
