...
// EC-121: Check whether user is able to enter Base URL or not
test(qase(121, 'EC-121: Check whether user is able to enter Base URL or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for Base URL field
  const baseUrlField = page.locator('input[name*="url"], input[id*="url"], input[name*="baseUrl"], input[placeholder*="URL"]').first();

  if (await baseUrlField.count() > 0) {
    await baseUrlField.fill('https://test.example.com/api');
    const value = await baseUrlField.inputValue();
    expect(value).toBe('https://test.example.com/api');
  }
});
