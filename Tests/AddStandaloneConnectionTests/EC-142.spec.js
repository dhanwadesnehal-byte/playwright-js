...
// EC-142: Check whether Connection Name field has character limit or not
test(qase(142, 'EC-142: Check whether Connection Name field has character limit or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for Connection Name field
  const connectionNameField = page.locator('input[name*="name"], input[id*="name"]').first();

  if (await connectionNameField.count() > 0) {
    // Check for maxlength attribute
    const maxLength = await connectionNameField.getAttribute('maxlength');

    // Try entering a very long name
    const longName = 'A'.repeat(500);
    await connectionNameField.fill(longName);
    const value = await connectionNameField.inputValue();

    // Value should either be truncated or limited
    if (maxLength) {
