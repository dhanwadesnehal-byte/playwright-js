...
// EC-113: Check whether user is able to see Add Standalone Connection form/modal or not
test(qase(113, 'EC-113: Check whether user is able to see Add Standalone Connection form/modal or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for form or modal
  const form = page.locator('form, [class*="modal"], [class*="dialog"], [role="dialog"]').first();
