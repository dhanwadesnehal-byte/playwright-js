...
// EC-117: Check whether user is able to enter Description or not
test(qase(117, 'EC-117: Check whether user is able to enter Description or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for Description field
  const descriptionField = page.locator('textarea[name*="description"], textarea[id*="description"], input[name*="description"], textarea').first();

  if (await descriptionField.count() > 0) {
    await descriptionField.fill('This is a test description for the standalone connection');
    const value = await descriptionField.inputValue();
    expect(value).toBe('This is a test description for the standalone connection');
  }
});
