...
// EC-125: Check whether user is able to enter Client Secret or not
test(qase(125, 'EC-125: Check whether user is able to enter Client Secret or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for Client Secret field
  const clientSecretField = page.locator('input[name*="secret"], input[id*="secret"], input[name*="clientSecret"]').first();

  if (await clientSecretField.count() > 0) {
    await clientSecretField.fill('test-client-secret-xyz789');
    const value = await clientSecretField.inputValue();
    expect(value).toBe('test-client-secret-xyz789');
  }
});
