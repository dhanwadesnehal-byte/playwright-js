...
// EC-141: Check whether Connection Name field accepts special characters or not
test(qase(141, 'EC-141: Check whether Connection Name field accepts special characters or not'), async ({ page }) => {
  ...
  if (await connectionNameField.count() > 0) {
    const specialCharName = 'Test@Connection#123!$%';
    await connectionNameField.fill(specialCharName);
    const value = await connectionNameField.inputValue();
    expect(value).toBe(specialCharName);
  }
});
