...
// EC-133: Check whether user is able to enter Key ID or not
test(qase(133, 'EC-133: Check whether user is able to enter Key ID or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for Key ID field
  const keyIdField = page.locator('input[name*="keyId"], input[id*="keyId"], input[name*="kid"]').first();

  if (await keyIdField.count() > 0) {
    await keyIdField.fill('test-key-id-abc123');
    const value = await keyIdField.inputValue();
    expect(value).toBe('test-key-id-abc123');
  }
});
