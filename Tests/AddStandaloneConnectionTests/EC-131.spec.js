...
// EC-131: Check whether user is able to enter Private Key or not
test(qase(131, 'EC-131: Check whether user is able to enter Private Key or not'), async ({ page }) => {
  await openAddStandaloneForm(page);

  // Look for Private Key field
  const privateKeyField = page.locator('textarea[name*="private"], textarea[id*="private"], textarea[name*="key"], input[name*="privateKey"]').first();

  if (await privateKeyField.count() > 0) {
    const testPrivateKey = '-----BEGIN PRIVATE KEY-----\nMIIEvQIBADANBgkqhkiG9w0BAQE...\n-----END PRIVATE KEY-----';
    await privateKeyField.fill(testPrivateKey);
    const value = await privateKeyField.inputValue();
    expect(value).toContain('PRIVATE KEY');
  }
});
