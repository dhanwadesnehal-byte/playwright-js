...
// EC-554: Check whether the Back button is displayed on the page
test(qase(554, 'EC-554: Check whether the Back button is displayed on the page'), async ({ page }) => {
  await openAddEmbedForm(page);
  await page.waitForTimeout(2000);

  // Look for Back button
  const backButton = page.locator('button:has-text("Back"), a:has-text("Back"), [class*="back"], button[aria-label*="Back" i]').first();

  if (await backButton.count() > 0) {
    await expect(backButton).toBeVisible();
    console.log('Back button is displayed on the page');
  } else {
    console.log('Back button not found - may not be present at this step');
