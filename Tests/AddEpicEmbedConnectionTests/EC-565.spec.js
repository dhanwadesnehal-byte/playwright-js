...
// EC-565: Check whether the Client ID field accepts valid input
test(qase(565, 'EC-565: Check whether the Client ID field accepts valid input'), async ({ page }) => {
  ...
  if (await clientIdField.count() > 0) {
    const testInput = 'test-client-id-12345';
    await clientIdField.fill(testInput);
    await page.waitForTimeout(500);

    const actualValue = await clientIdField.inputValue();

    if (actualValue === testInput) {
      expect(actualValue).toBe(testInput);
      console.log('Client ID field accepts valid input');
    } else {
      console.log(`Client ID field value: ${actualValue}`);
