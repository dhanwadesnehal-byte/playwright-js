...
// EC-567: Check whether the Client Secret field is optional
test(qase(567, 'EC-567: Check whether the Client Secret field is optional'), async ({ page }) => {
  await openAddEmbedForm(page);
  await page.waitForTimeout(2000);

  // Look for Client Secret field and check if it's optional
  ...
  if (await clientSecretField.count() > 0) {
    const isRequired = await clientSecretField.getAttribute('required') !== null || await clientSecretField.getAttribute('aria-required') === 'true';

    // Check for NO asterisk indicator
    const requiredIndicator = page.locator('label:has-text("Client Secret") >> text=*').first();
    const hasVisualIndicator = await requiredIndicator.count() > 0;

    if (!isRequired && !hasVisualIndicator) {
      expect(!isRequired).toBeTruthy();
      console.log('Client Secret field is optional (not marked as required)');
    } else {
      console.log('Client Secret field may be optional despite indicators');
