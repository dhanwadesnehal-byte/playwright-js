...
// EC-559: Check whether the Connection Name field accepts valid alphanumeric input
test(qase(559, 'EC-559: Check whether the Connection Name field accepts valid alphanumeric input'), async ({ page }) => {
  ...
  if (await connectionNameField.count() > 0) {
    const testInput = 'TestConnection123';
    await connectionNameField.fill(testInput);
    await page.waitForTimeout(500);

    const actualValue = await connectionNameField.inputValue();

    if (actualValue === testInput) {
      expect(actualValue).toBe(testInput);
      console.log('Connection Name field accepts valid alphanumeric input');
    } else {
      console.log(`Connection Name field value: ${actualValue}`);
