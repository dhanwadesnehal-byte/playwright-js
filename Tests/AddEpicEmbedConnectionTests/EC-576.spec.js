...
// EC-576: Check whether the OAuth Callback URL field is read-only
test(qase(576, 'EC-576: Check whether the OAuth Callback URL field is read-only'), async ({ page }) => {
  await openAddEmbedForm(page);
  await page.waitForTimeout(2000);

  // Look for OAuth Callback URL field
  const oauthCallbackUrlField = page.locator('input[name*="oauth" i][name*="callback" i], input[name*="callback" i][name*="url" i], input[id*="oauth" i][id*="callback" i], input[placeholder*="OAuth Callback URL" i], input[placeholder*="Callback URL" i]').first();

  if (await oauthCallbackUrlField.count() > 0) {
    const isReadonly = await oauthCallbackUrlField.getAttribute('readonly') !== null || await oauthCallbackUrlField.getAttribute('disabled') !== null;

    if (isReadonly) {
      expect(isReadonly).toBeTruthy();
      console.log('OAuth Callback URL field is read-only');
    } else {
      console.log('OAuth Callback URL field found but may not be read-only yet');
