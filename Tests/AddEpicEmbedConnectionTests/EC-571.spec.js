...
// EC-571: Check whether the Embedded App URL field accepts a valid URL format
test(qase(571, 'EC-571: Check whether the Embedded App URL field accepts a valid URL format'), async ({ page }) => {
  ...
  if (await embeddedAppUrlField.count() > 0) {
    ...
    await embeddedAppUrlField.fill(testUrl);
    await page.waitForTimeout(500);

    const actualValue = await embeddedAppUrlField.inputValue();

    if (actualValue === testUrl) {
      expect(actualValue).toBe(testUrl);
      console.log('Embedded App URL field accepts valid URL format');
    } else {
      console.log(`Embedded App URL field value: ${actualValue}`);
