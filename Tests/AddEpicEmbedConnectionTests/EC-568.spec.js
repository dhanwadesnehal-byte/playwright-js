...
// EC-568: Check whether the Client Secret value is masked when entered
test(qase(568, 'EC-568: Check whether the Client Secret value is masked when entered'), async ({ page }) => {
  ...
  if (await clientSecretField.count() > 0) {
    // Check if field type is password (which masks input)
    const inputType = await clientSecretField.getAttribute('type');

    if (inputType === 'password') {
      expect(inputType).toBe('password');
      console.log('Client Secret value is masked (input type is password)');
    } else {
      console.log(`Client Secret field type: ${inputType}`);
