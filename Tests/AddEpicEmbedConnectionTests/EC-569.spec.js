...
// EC-569: Check whether helper text for Client Secret is displayed
test(qase(569, 'EC-569: Check whether helper text for Client Secret is displayed'), async ({ page }) => {
  await openAddEmbedForm(page);
  await page.waitForTimeout(2000);

  // Look for helper text near Client Secret field
  const helperText = page.locator('[class*="help"]:visible, [class*="hint"]:visible, [class*="description"]:visible, small:visible, [role="note"]:visible').first();

  if (await helperText.count() > 0) {
    await expect(helperText).toBeVisible();
    console.log('Helper text for Client Secret is displayed');
  } else {
    console.log('Helper text may not be present or visible at this step');
