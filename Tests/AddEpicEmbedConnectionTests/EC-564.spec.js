...
// EC-564: Check whether a validation message is displayed when Client ID is left empty
test(qase(564, 'EC-564: Check whether a validation message is displayed when Client ID is left empty'), async ({ page }) => {
  await openAddEmbedForm(page);
  await page.waitForTimeout(2000);

  // Look for Client ID field and leave it empty
  ...
  if (await clientIdField.count() > 0) {
    ...
    await clientIdField.blur();
    await page.waitForTimeout(1000);

    // Look for validation message
    const validationMessage = page.locator('[class*="error"]:visible, [class*="invalid"]:visible, [role="alert"]:visible, .error-message:visible, span:has-text("required"):visible').first();

    if (await validationMessage.count() > 0) {
      await expect(validationMessage).toBeVisible();
      console.log('Validation message is displayed when Client ID is left empty');
    } else {
      console.log('Validation message may be displayed on submit attempt');
